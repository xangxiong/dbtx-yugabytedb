...
from dbt.adapters.yugabytedb.relation_configs.constants import MAX_CHARACTERS_IN_IDENTIFIER
from dbt.adapters.yugabytedb.relation_configs.index import (
...
class YugabytedbMaterializedViewConfig(RelationConfigBase, RelationConfigValidationMixin):
    """
    ...
    table_name: str = ""
    query: str = ""
    indexes: FrozenSet[YugabytedbIndexConfig] = field(default_factory=frozenset)
    ...
    @classmethod
    def from_dict(cls, config_dict: dict) -> "YugabytedbMaterializedViewConfig":
        kwargs_dict = {
            "table_name": config_dict.get("table_name"),
            "query": config_dict.get("query"),
            "indexes": frozenset(
                YugabytedbIndexConfig.from_dict(index) for index in config_dict.get("indexes", {})
            ),
        }
        materialized_view: "YugabytedbMaterializedViewConfig" = super().from_dict(kwargs_dict)  # type: ignore
        return materialized_view

    @classmethod
    def from_model_node(cls, model_node: ModelNode) -> "YugabytedbMaterializedViewConfig":
    ...
    def parse_model_node(cls, model_node: ModelNode) -> dict:
        ...
        config_dict = {
            ...
            "indexes": [YugabytedbIndexConfig.parse_model_node(index) for index in indexes],
        }
        return config_dict
    ...
    def parse_relation_results(cls, relation_results: RelationResults) -> dict:
        ...
        config_dict = {
            "indexes": [
                YugabytedbIndexConfig.parse_relation_results(index) for index in indexes.rows
            ],
        }
        return config_dict
...
@dataclass
class YugabytedbMaterializedViewConfigChangeCollection:
    indexes: Set[YugabytedbIndexConfigChange] = field(default_factory=set)
