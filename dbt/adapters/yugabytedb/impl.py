...
from dbt.adapters.yugabytedb import YugabytedbConnectionManager
from dbt.adapters.yugabytedb.column import YugabytedbColumn
from dbt.adapters.yugabytedb import YugabytedbRelation
...
GET_RELATIONS_MACRO_NAME = "yugabytedb__get_relations"


@dataclass
class YugabytedbIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["YugabytedbIndexConfig"]:
...
@dataclass
class YugabytedbConfig(AdapterConfig):
    unlogged: Optional[bool] = None
    indexes: Optional[List[YugabytedbIndexConfig]] = None


class YugabytedbAdapter(SQLAdapter):
    Relation = YugabytedbRelation
    ConnectionManager = YugabytedbConnectionManager
    Column = YugabytedbColumn

    AdapterSpecificConfigs = YugabytedbConfig
    ...
    @available
    def parse_index(self, raw_index: Any) -> Optional[YugabytedbIndexConfig]:
        return YugabytedbIndexConfig.parse(raw_index)
    ...
    def _get_catalog_schemas(self, manifest):
        # yugabytedb only allow one database (the main one)
